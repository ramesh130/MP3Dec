...
import javazoom.jl.xmlparser.XMLParser;
...
public class jlc
{
	...
	static public void main(String args[])
	{
		...
		jlcArgs ma = new jlcArgs(new XMLParser());
		if (!ma.processArgs(argv))
			System.exit(1);

		int stnCount = ma.getStationCount();
		...
		Converter[] thrArray = new Converter[stnCount];
		//create a thread for each station
		for(int i = 0; i < stnCount; i++){
			Converter conv = new Converter(ma.getInputURL(i), ma.getOutputFilename(i), listener, 8000, 1, 8);
			conv.start();
			thrArray[i] = conv;
		}
		...
		while(true){
			//loop infinitely
			try {
				Thread.sleep(120*1000);//10000
				for(int i = 0; i < stnCount; i++){
					if(!thrArray[i].getHeartIsBeating()){
						thrArray[i].interrupt();
						Converter conv = new Converter(ma.getInputURL(i), ma.getOutputFilename(i), listener, 8000, 1, 8);
						conv.start();
						thrArray[i] = conv;
					}else{
						thrArray[i].resetHeartIsBeating();
					}
				}
	...
	static class jlcArgs
	{
		...
		public jlcArgs(XMLParser parser)
		{
		...
		public String getInputURL(int i) {
			// TODO Auto-generated method stub
			return parser.getURL(i);
		}

		public String getOutputFilename(int i) {
			// TODO Auto-generated method stub
			return parser.getFileName(i);
		}

		public int getStationCount() {
			// TODO Auto-generated method stub
			return parser.getStationCount();
		}
		...
		public boolean processArgs(String[] argv)
		{
			...
			//			return Usage();
			parser.parse();
//			filename = parser.getURL(0);
//			output_filename = parser.getFileName(0);
