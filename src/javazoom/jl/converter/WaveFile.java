...
public class WaveFile extends RiffFile
{
   ...
   public int SeekToSample ( long SampleIndex )
   {
	...
	private AudioFormat outDataFormat = new AudioFormat((float) 8000.0,
			(int) 16, (int) 1, true, false);//@TODO:remove hard coding

	private AudioFormat inDataFormat = new AudioFormat((float) 22050,
			(int) 16, (int) 1, true, false);//@TODO:remove hard coding
	...
	public int WriteData ( short[] data, int numData )
	{
		...
		byte[] byteData = new byte[extraBytes];
		int yc = 0;
		for (int y = 0;y<extraBytes;y=y+2)
		{
			byteData[y] = (byte) (data[yc] & 0x00FF);
			byteData[y+1] =(byte) ((data[yc++] >>> 8) & 0x00FF);
		}
		
		InputStream inStream = new ByteArrayInputStream(byteData);
		AudioInputStream ainStream = new AudioInputStream(inStream, inDataFormat, extraBytes);

		AudioInputStream lowResAIS;
		boolean isSUpported = AudioSystem.isConversionSupported(outDataFormat,inDataFormat);
		int len = 0;
		if(isSUpported){
			lowResAIS = AudioSystem.getAudioInputStream(outDataFormat, ainStream);
			try {
				len = (int)lowResAIS.getFrameLength();
				//len is frame size...convert it to bytes
				//frameSize = (sampleSizeInBits + 7) / 8 * channels;				
				//int frameSize = len * ((int)(16 + 7) / 8 * 1);//@TODO:Remove Hard coding
				
				byte[] decByteData = new byte[len];

				lowResAIS.read(decByteData, 0, len);
				
				//linear to ALAW
				DSPUtils.pcm162alaw(decByteData, 0, len>>1, false);		//converts to 8 bit as well :)		
				
				pcm_data.ckSize += len>>1;
				super.Write ( decByteData, len>>1 );
			} catch (IOException e) {
